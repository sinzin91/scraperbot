...
ScraperBot.prototype._replyWithPPSID = function (originalMessage, sku, store_id) {
    ...
    pool.getConnection(function(err, connection) {
        ...
        !self._isFromScraperBot(originalMessage)) {
            ...
            console.log('store id: ' + store_id[1]);
            connection.query("select id from products_per_store as pps where (pps.sku, store_id) = ('" + sku[1] + "', " + store_id[1] + ")", function (err, record) {
                var channel = self._getChannelById(originalMessage.channel);
                if (err) {
...
ScraperBot.prototype._replyWithSKU = function (originalMessage, ppsid, store_id) {
    ...
    pool.getConnection(function(err, connection) {
        if (self._isChannelConversation(originalMessage) &&
        !self._isFromScraperBot(originalMessage)) {
            console.log('id: ' + ppsid[1]);
            console.log('store id: ' + store_id[1]);
            connection.query("select sku from products_per_store as pps where (pps.id, store_id) = ('" + ppsid[1] + "', " + store_id[1] + ")", function (err, record) {
                var channel = self._getChannelById(originalMessage.channel);
                if (err) {
...
var help_message = 'Hi Wiseguy, I\'m here to help find scrapers! ' +
            ...
            '\nPlease ask Tenzin if you have any SQL requests you want me to handle'; 
