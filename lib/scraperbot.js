...
ScraperBot.prototype._replyWithScraperIds = function (originalMessage, store_id) {
    ...
    pool.getConnection(function(err, connection) {
        ...
        connection.query('SELECT crawl_store_id FROM external_crawl_settings WHERE store_id = ?', store_id, function (err, record) {
            if (err) {
...
ScraperBot.prototype._welcomeMessage = function () {
    this.postMessageToChannel(this.channels[0].name, 'Hi guys, I\'m here to help find scrapers! ' +
        '\nTo see what scrapers are attached to a store ID, just type "attached scrapers STORE_ID". ' +
        '\nTo find a scraper\'s name from scraper ID, just type "scraper name of SCRAPER_ID". ',
        {as_user: true});
};
