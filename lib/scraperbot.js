            ...
            _batchPull(list, store_id, hash_key, function(rows) {
                _writeToCSV(message, rows, store_id, hash_key, function(file) {
                    ...
                    if (this._isChannelConversation(message) &&
                        !this._isFromScraperBot(message)) {
                        var channel = this._getChannelById(message.channel);
                        this.postMessageToChannel(channel.name, "Boom!  Here's the CSV with all URLs in DynamoDB: http://54.193.12.26:8080/csvs/" + file, {as_user: true});
                    } else if (this._isDirectConversation(message) &&
                        !this._isFromScraperBot(message)) {
                        
                        this.postMessage(message.user, "Boom!  Here's the CSV with all URLs in DynamoDB: http://54.193.12.26:8080/csvs/" + file, {as_user: true});
                    }
                        ...
                        json2csv({ data: data }, function(err, csv) {
                            ...
                            fs.writeFile(file, csv, function(err) {
                                ...
                                if (this._isChannelConversation(message) &&
                                    ...
                                    !this._isFromScraperBot(message)) {
                                    this.postMessage(message.user, "Boom!  Here's a CSV with URLs with and without matches: http://54.193.12.26:8080/csvs/" + file, {as_user: true});
                                }
                            }.bind(this));
