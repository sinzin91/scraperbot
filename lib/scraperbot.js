...
ScraperBot.prototype._onMessage = function (message) {
    ...
    if (this._isChatMessage(message) &&
        ...
        console.log('Moment: ' + moment().format());
        librato.increment('ScraperBot._replyWithScraperIds');
    ...
    if (this._isChatMessage(message) &&
        ...
        librato.increment('ScraperBot._replyWithScraperId');
    ...
    if (this._isChatMessage(message) &&
        ...
        librato.increment('ScraperBot._replyWithScraperName');
    ...
    if (this._isChatMessage(message) &&
        ...
        librato.increment('ScraperBot._replyWithSearchResults');
    ...
    if (this._isChatMessage(message) &&
        ...
        librato.increment('ScraperBot._replyWithStoreIds');
    ...
    if (this._isChatMessage(message) &&
        ...
        librato.increment('ScraperBot._replyWithURL');
    ...
    if (this._isChatMessage(message) &&
        ...
        console.log(SKU); 
        console.log(store_id);
        librato.increment('ScraperBot._replyWithPPSID');
    ...
    if (this._isChatMessage(message) &&
        ...
        librato.increment('ScraperBot._pullAllURLs');
        ...
        this._pullPPSIDs(message, store_id, hash_key, function(list) {
            ...
            _batchPull(list, store_id, hash_key, function(rows) {
                if (rows[0] == null && (this._isChannelConversation(message) &&
                    ...
                    var channel = this._getChannelById(message.channel);
                    this.postMessageToChannel(channel.name, "Sorry brah, either there are no URLs uploaded, or its a legacy scraper, not Superminer. :disappointed:", {as_user: true});
                } else if (rows[0] == null && (this._isDirectConversation(message) &&
                            !this._isFromScraperBot(message))) {
                    console.log("ROWS EMPTY");
                    this.postMessage(message.user, "Sorry brah, either there are no URLs uploaded, or its a legacy scraper, not Superminer. :disappointed:", {as_user: true});
                } else {
    ...
    if (this._isChatMessage(message) &&
        ...
        librato.increment('ScraperBot._pullAllURLsWithMatches');
        if (this._isChannelConversation(message) &&
            !this._isFromScraperBot(message)) {
            var channel = this._getChannelById(message.channel);
        ...
        this._pullPPSIDs(message, store_id, hash_key, function(list) {
            ...
            _batchPull(list, store_id, hash_key, function(rows) {
                ...
                var compDomain = "%www." + hash_key[1].match(/[^\_]*/)[0].toString() + "%";
    ...
    if (this._isChatMessage(message) &&
        ...
        librato.increment('ScraperBot._helpMessage');
    }
    ...
    if (this._isChatMessage(message) &&
        ...
        librato.increment('ScraperBot._noResultsMessage');
    }
    ...
    pool.getConnection(function(err, connection) {
        connection.query('SELECT pps.id FROM products_per_store AS pps WHERE pps.store_id = ?', store_id[1], function (err, records) {
            ...
            records.forEach(function(record){
                list.push(record.id + '.Scraper_' + scraper_class_name[1].replace('.',''));
            });
            callback(list)
...
// Data models
var help_message = 'Hi Wiseguy, I\'m here to find scrapers and perform SQL queries so you don\'t ahve to! ' +
            '\n' +        
            '\n*Commands:* ' +
