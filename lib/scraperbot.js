 ...
 * console output execution timestamp
...
ScraperBot.prototype._onMessage = function (message) {
    ...
    if (this._isChatMessage(message) &&
        ...
        this._pullPPSIDs(message, store_id, hash_key, function(list) {
            ...
            _batchPull(list, function(rows) {
                _writeToCSV(message, rows, function() {
                    console.log('bingo, in callback now!');
                    this.postMessage(message.user, "Boom!  Here's the CSV with all URLs in DynamoDB: http://127.0.0.1:8080/out.csv", {as_user: true});
...
function _batchPull(list, callback) {
    ...
    Promise.all(promises).then(function(results) {
        ...
        rows = results.map(function(result) {
            var row = [];
            row.push(result.sku, result.updated_url);
            return row;
        });
        callback(rows);
    });
...
function _writeToCSV(originalMessage, rows, callback) {
    console.log('made it to writeToCSV');
    console.log('Rows: ' + rows);
    console.log('WRITING TO CSV!!!!');
    var writer = csv({ headers: ["sku", "url"]});
